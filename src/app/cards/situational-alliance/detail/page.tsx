...
function situationalAllianceToCard(sa: ISituationalAlliance): Card {
  return {
    id: sa.id,
    name: sa.name,
    type: 'SA',
    ...
    factionLeftLogoVersion: sa.factionLeftLogoVariant as Card['factionLeftLogoVersion'],
    factionRight: sa.factionRight,
    factionRightLogoVersion: sa.factionRightLogoVariant as Card['factionRightLogoVersion'],
    rarity: 'Common',
...
function SituationalAllianceDetailContent() {
  ...
  useEffect(() => {
    ...
    apiService.getSituationalAllianceById(id).then(sa => {
      setCard(sa ? situationalAllianceToCard(sa) : null);
      setLoading(false);
    });
  }, [id]);
    ...
    >
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1">
          ...
          <div className="p-2 rounded" style={{ background: '#111608', border: '1px solid #4a5e35' }}>
            <div className="grid grid-cols-2 gap-2 text-center">
              ...
              <div>
                <div className="font-mono uppercase tracking-wider text-xs mb-0.5" style={{ color: '#6a7a5a' }}>Custo</div>
                <div className="font-bold text-xs px-1 py-0.5 rounded" style={{ background: '#0d1208', color: '#c9a84c', border: '1px solid #3a4a2a' }}>{card.cost}</div>
              </div>
            </div>
          </div>
      ...
      <div className={`flex flex-col items-center justify-center ${isDesktop ? 'flex-1 overflow-hidden' : 'w-full min-h-[300px]'}`}>
        ...
        <div className="w-full flex-1 flex items-center justify-center">
          {isDesktop ? (
            <CardDesktopSituationalAlliance selectedCard={card} isFlipped={isFlipped} getFactionLogo={getFactionLogo} />
          ) : (
