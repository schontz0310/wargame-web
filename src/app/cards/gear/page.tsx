...
import { IGear, GearsFilters, apiService } from '@/lib/api'
...
function GearListContent() {
  ...
  const [cards, setCards] = useState<IGear[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [total, setTotal] = useState(0);
  const [totalPages, setTotalPages] = useState(1);

  const [filters, setFilters] = useState<GearsFilters>({ page: 1, limit: 20 });
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    const f: GearsFilters = {
      ...filters,
      search: search || undefined,
      faction: selectedFaction || undefined,
      expansion: selectedExpansion || undefined,
      class: selectedClass || undefined,
      attachesTo: selectedAttachesTo || undefined,
    };
    apiService.getGears(f)
      .then(res => {
        setCards(res.gears);
        setTotal(res.total);
        setTotalPages(res.totalPages);
        const uniqueFactions = [...new Set(res.gears.map(c => c.faction).filter((x): x is string => !!x))].sort();
        setFactions(prev => [...new Set([...prev, ...uniqueFactions])].sort());
        const uniqueExpansions = [...new Set(res.gears.map(c => c.expansion))].sort();
        setExpansions(prev => [...new Set([...prev, ...uniqueExpansions])].sort());
      })
      .catch(err => setError(err.message))
      .finally(() => setLoading(false));
  }, [filters, search, selectedFaction, selectedExpansion, selectedClass, selectedAttachesTo]);
    ...
    <div className="flex h-screen" style={{background:'linear-gradient(160deg,#080c05 0%,#0d1208 60%,#0a0f06 100%)'}}>
      ...
      <div className="w-56 flex flex-col h-full overflow-y-auto flex-shrink-0" style={{background:'#0a0f06',borderRight:'1px solid #2a3a1a'}}>
        ...
        <div className="p-3 space-y-4 flex-1">
          <form onSubmit={handleSearch}>
            <label className="block font-mono text-xs mb-1" style={{color:'#5a7a4a'}}>BUSCAR NOME/EFEITO</label>
            <input
              type="text"
              value={search}
          ...
          <div>
            <label className="block font-mono text-xs mb-1" style={{color:'#5a7a4a'}}>ANEXA A</label>
            <select
              value={selectedAttachesTo}
              onChange={e => { setSelectedAttachesTo(e.target.value); setFilters(f => ({ ...f, page: 1 })); }}
              className="w-full px-2 py-1.5 text-xs font-mono"
              style={{background:'rgba(0,0,0,0.4)',border:'1px solid #3a4a2a',color:'#c9a84c'}}
            >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-4 py-3 flex items-center justify-between" style={{background:'rgba(0,0,0,0.5)',borderBottom:'1px solid #3a4a2a'}}>
          <div>
            <h1 className="font-mono text-sm font-bold tracking-widest uppercase" style={{color:'#c9a84c'}}>CARTAS — GEAR</h1>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10" style={{background:'rgba(10,15,6,0.97)',borderBottom:'1px solid #2a3a1a'}}>
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left text-xs font-mono" style={{color:'#5a7a4a',borderRight:'1px solid #1a2a10'}}>ANEXA A</th>
                  ...
                  {cards.map(card => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-xs font-mono font-medium max-w-[220px] truncate" style={{color:'#e8d5a0',borderRight:'1px solid #1a2a10'}}>
                        {card.name}
                        {card.isSingleUse && (
                          <span className="ml-1 font-mono uppercase text-[9px] px-1 py-0.5 rounded" style={{background:'#1a2410',color:'#c9a84c',border:'1px solid #4a5e35'}}>Uso Único</span>
                        )}
                      </td>
