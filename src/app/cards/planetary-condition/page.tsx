...
function PlanetaryConditionListContent() {
  const router = useRouter();
  const t = useT();

  const [allCards, setAllCards] = useState<IPlanetaryCondition[]>([]);
  ...
  // set is fetched once (100 per page, the backend's max) and paginated client-side.
  useEffect(() => {
    setLoading(true);
    setError(null);
    apiService.getPlanetaryConditions({ page: 1, limit: 100 })
      .then(async res => {
        let cards = res.planetaryConditions;
        if (res.totalPages > 1) {
          const rest = await Promise.all(
            Array.from({ length: res.totalPages - 1 }, (_, i) => apiService.getPlanetaryConditions({ page: i + 2, limit: 100 }))
          );
          cards = cards.concat(...rest.map(r => r.planetaryConditions));
        }
        setAllCards(cards);
      })
    ...
    <div className="flex h-screen" style={{background:'linear-gradient(160deg,#080c05 0%,#0d1208 60%,#0a0f06 100%)'}}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-4 py-3 flex items-center justify-between" style={{background:'rgba(0,0,0,0.5)',borderBottom:'1px solid #3a4a2a'}}>
          <div>
            <h1 className="font-mono text-sm font-bold tracking-widest uppercase" style={{color:'#c9a84c'}}>{t('cardsUI.cardsHeader')} — PLANETARY CONDITION</h1>
                ...
                <tbody>
                  {pageCards.map(card => (
                    <tr
                      ...
                      onClick={() => router.push(`/cards/planetary-condition/detail?id=${card.id}`)}
                    >
                      ...
                      <td className="px-3 py-2 text-xs font-mono max-w-md truncate" style={{color:'#6a8a4a',borderRight:'1px solid #1a2a10'}}>{card.description}</td>
                      ...
                      <td className="px-3 py-2" style={{borderRight:'1px solid #1a2a10'}}>
                        <CardCollectionButtons card={{ id: card.id, cardId: card.cardId, name: card.name, cardType: 'PC', expansion: card.expansion, collectionNumber: card.collectionNumber }} onChange={() => setCollectionVersion(v => v + 1)} />
                      </td>
                      <td className="px-3 py-2 text-center">
                        <button
                          onClick={e => { e.stopPropagation(); router.push(`/cards/planetary-condition/detail?id=${card.id}`); }}
...
export default function PlanetaryConditionPage() {
  const t = useT();
  return (
