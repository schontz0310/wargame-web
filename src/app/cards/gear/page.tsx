...
const ATTACHES_TO = ['Attack', 'Ballistic', 'Melee', 'Energy', 'Defense', 'Speed', 'Damage'];

function GearListContent() {
  const router = useRouter();
  const t = useT();

  const [allCards, setAllCards] = useState<IGear[]>([]);
  ...
  const [selectedAttachesTo, setSelectedAttachesTo] = useState('');
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    apiService.getGears({ page: 1, limit: 100 })
      .then(async res => {
        let cards = res.gears;
        if (res.totalPages > 1) {
          const rest = await Promise.all(
            Array.from({ length: res.totalPages - 1 }, (_, i) => apiService.getGears({ page: i + 2, limit: 100 }))
          );
          cards = cards.concat(...rest.map(r => r.gears));
        }
        setAllCards(cards);
      })
  ...
  const factions = useMemo(() => [...new Set(allCards.map(c => c.faction).filter((x): x is string => !!x))].sort(), [allCards]);
  ...
  const filteredCards = useMemo(() => {
    ...
    return allCards.filter(c => {
      ...
      if (selectedAttachesTo && c.attachesTo !== selectedAttachesTo) return false;
  ...
  }, [allCards, search, selectedFaction, selectedExpansion, selectedClass, selectedAttachesTo, showHaveOnly, showWantOnly, collectionVersion]);
    ...
    <div className="flex h-screen" style={{background:'linear-gradient(160deg,#080c05 0%,#0d1208 60%,#0a0f06 100%)'}}>
      ...
      <div className="w-56 flex flex-col h-full overflow-y-auto flex-shrink-0" style={{background:'#0a0f06',borderRight:'1px solid #2a3a1a'}}>
        ...
        <div className="p-3 space-y-4 flex-1">
          ...
          <div>
            <label className="block font-mono text-xs mb-1" style={{color:'#5a7a4a'}}>{t('cardsUI.attachesTo')}</label>
            <select
              value={selectedAttachesTo}
              onChange={e => { setSelectedAttachesTo(e.target.value); setPage(1); }}
              className="w-full px-2 py-1.5 text-xs font-mono"
              style={{background:'rgba(0,0,0,0.4)',border:'1px solid #3a4a2a',color:'#c9a84c'}}
            >
              <option value="">{t('cardsUI.allM')}</option>
              {ATTACHES_TO.map(at => <option key={at} value={at}>{at}</option>)}
            </select>
          </div>
          <div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-4 py-3 flex items-center justify-between" style={{background:'rgba(0,0,0,0.5)',borderBottom:'1px solid #3a4a2a'}}>
          <div>
            <h1 className="font-mono text-sm font-bold tracking-widest uppercase" style={{color:'#c9a84c'}}>{t('cardsUI.cardsHeader')} — GEAR</h1>
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 z-10" style={{background:'rgba(10,15,6,0.97)',borderBottom:'1px solid #2a3a1a'}}>
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left text-xs font-mono" style={{color:'#5a7a4a',borderRight:'1px solid #1a2a10'}}>{t('cardsUI.colAttachesTo')}</th>
                ...
                <tbody>
                  {pageCards.map(card => (
                    <tr
                      ...
                      onClick={() => router.push(`/cards/gear/detail?id=${card.id}`)}
                    >
                      ...
                      <td className="px-3 py-2 text-xs font-mono font-medium max-w-[220px] truncate" style={{color:'#e8d5a0',borderRight:'1px solid #1a2a10'}}>
                        {card.name}
                        {card.isSingleUse && (
                          <span className="ml-1 font-mono uppercase text-[9px] px-1 py-0.5 rounded" style={{background:'#1a2410',color:'#c9a84c',border:'1px solid #4a5e35'}}>{t('cardsUI.singleUse')}</span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-xs font-mono" style={{color:'#7a9a5a',borderRight:'1px solid #1a2a10'}}>{card.attachesTo || '—'}</td>
                      <td className="px-3 py-2 text-xs font-mono" style={{color:'#7a9a5a',borderRight:'1px solid #1a2a10'}}>{card.faction || '—'}</td>
                      ...
                      <td className="px-3 py-2 text-xs font-mono text-center" style={{borderRight:'1px solid #1a2a10'}}>
                        <span className="px-1.5 py-0.5 font-bold" style={{background:'rgba(201,168,76,0.15)',border:'1px solid #c9a84c55',color:'#c9a84c'}}>{card.points}</span>
                      </td>
                      ...
                      <td className="px-3 py-2" style={{borderRight:'1px solid #1a2a10'}}>
                        <CardCollectionButtons card={{ id: card.id, cardId: card.cardId, name: card.name, cardType: 'G', expansion: card.expansion, collectionNumber: card.collectionNumber }} onChange={() => setCollectionVersion(v => v + 1)} />
                      </td>
                      <td className="px-3 py-2 text-center">
                        <button
                          onClick={e => { e.stopPropagation(); router.push(`/cards/gear/detail?id=${card.id}`); }}
